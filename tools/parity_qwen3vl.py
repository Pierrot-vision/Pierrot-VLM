#!/usr/bin/env python
"""Qwen3-VL 스크래치 구현 vs HF 공식 구현 수치 parity 검증 (opt-in).

기본 테스트(tests/)는 다운로드 불필요 원칙이라 실제 2B 가중치 비교를 포함하지 않는다.
이 스크립트가 그 간극을 메운다 — attention/M-RoPE/전처리를 손대면 이걸 돌려 회귀를 잡는다.

요구사항(충족 못 하면 이유를 출력하고 skip 종료):
  - transformers >= 4.57 (Qwen3VLForConditionalGeneration 지원)
  - Qwen/Qwen3-VL-2B-Instruct 다운로드 가능(HF Hub 또는 로컬 캐시)
  - RAM: 두 모델을 **순차** 로드(공식 → 해제 → 커스텀)하므로 fp32 기준 peak 약 12~16GB.
    (동시 로드였다면 20GB+ — 그래서 순차로 구성했다. 여유가 24GB 이상이면 안전.)

실행:
    python tools/parity_qwen3vl.py                       # 기본(2B, CPU, fp32)
    python tools/parity_qwen3vl.py --model <id/경로> --device cuda

검증 항목(모두 통과해야 exit 0):
  ① 전처리: input_ids 완전 동일 · image_grid_thw 동일 · pixel_values max|diff| ≤ 2/255
  ② 비전 타워(동일 픽셀 입력): 머저·DeepStack 출력 max|diff| ≤ 1e-3
...
# ------------------------------------------------------------------ #
# 요구사항 검사: transformers 버전이 Qwen3-VL 을 모르면 skip(실패 아님) 종료.
# ------------------------------------------------------------------ #
def _require_hf_qwen3vl():
    import transformers
    try:
        from transformers import Qwen3VLForConditionalGeneration  # noqa: F401
    except ImportError:
        print(f"[skip] transformers {transformers.__version__} 는 Qwen3-VL 을 지원하지 않습니다 "
              f"(>=4.57 필요). parity 는 별도 환경에서 실행하세요.")
        sys.exit(0)
...
# (두 2B 모델을 동시에 들고 있지 않기 위한 순차 실행 — peak RAM 절반)
# ------------------------------------------------------------------ #
def _run_official(model_id: str, device: str, image: Image.Image, max_new_tokens: int) -> dict:
    ...
    with torch.no_grad():
        ...
        merged, deep = ((vis[0], vis[1]) if isinstance(vis, tuple)
                        else (vis.pooler_output, vis.deepstack_features))
    ...
    ref = {
        ...
        "vision_deep":    [d.cpu() for d in deep],
        "logits":         logits.cpu(),
    ...
    del model, inputs, vis, merged, deep, logits, gen
    gc.collect()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Qwen3-VL parity 검증(opt-in)")
    parser.add_argument("--model", default=MODEL_DEFAULT)
    parser.add_argument("--device", default="cpu")
    parser.add_argument("--max-new-tokens", type=int, default=32)
    args = parser.parse_args()

    ver = _require_hf_qwen3vl()
    ...
    # ---------- 2단계: 커스텀 구현 로드 ----------
    from pierrot.models.qwen3vl.weights import load_pretrained
    ...
    print("\n== ② 비전 타워 ==")
    with torch.no_grad():
        our_merged, our_deep = our_model.model.visual(
    ...
    for i, (a, b) in enumerate(zip(ref["vision_deep"], our_deep)):
        di = float((a - b.cpu()).abs().max())
        check(f"DeepStack[{i}] ≤ 1e-3", di <= 1e-3, f"max|diff|={di:.6f}")
